...
class SolarDataExtractor:
    ...
    def extract_from_df(self, df: pd.DataFrame, table_name: str, mode: str = "replace"):
    ...
    if df_upload is not None:
        ...
        if not st.session_state.get("colmap_confirmed", False):
            ...
            **Your Data Format Detected:** Standard solar monitoring export
            ...
            def safe_idx(name):
                try:
                    return cols_list.index(name) + 1 if name else 0
                except ValueError:
                    return 0
        ...
        if show_save_ui:
            ...
            if st.button("💾 Save to Database", type="primary"):
                if df_to_save is None or df_to_save.empty:
                    ...
                    try:
                        ok, msg = extractor.extract_from_df(df_to_save, table_name, mode)
                        if ok:
                            st.success(msg)
                        else:
                            st.error(msg)
...
with tab3:
    ...
    if not tables:
        ...
        # Table selector
        selected_table = st.selectbox("Select table to manage", tables, key="tab3_table_select")
...
with tab4:
    ...
    if not tables:
        ...
        sel_tbl = st.selectbox("Select Table", tables, key="t4_tbl")
...
with tab5:
    ...
    if not tables:
        ...
        calc_tbl = st.selectbox("Source Table", tables, key='t5_sel')
...
with tab6:
    ...
    if not tables:
        ...
        source_tbl = st.selectbox("Select Data Table", tables, key='t6_sel_tbl')
